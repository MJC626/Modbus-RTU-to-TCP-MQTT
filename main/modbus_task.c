...
void modbus_poll_task(void *pvParameters) {
    ...
    while(1) {
        ...
        for(int i = 0; i < modbus_config.group_count; i++) {
            // 如果当前组没有启用或者 UART 端口不匹配，跳过该组
            if(!modbus_config.groups[i].enabled ||
               modbus_config.groups[i].uart_port != mb_ctx->uart_port) {
                continue;
            }
            ...
            if (send_len > 0) {
                ...
                if (read_len > 0) {
                    ...
                    switch(modbus_config.groups[i].function_code) {
                        case 1: { // Read Coils
                            uint8_t bit_values[MAX_BITS/8] = {0};
                            rc = agile_modbus_deserialize_read_bits(ctx, read_len, bit_values);
                            if (rc >= 0) {
                                // 计算需要存储的字节数 (向上取整)
                                int byte_count = (modbus_config.groups[i].reg_count + 7) / 8;
                                memcpy(modbus_data.coils[i], bit_values, byte_count);
                            }
                            break;
                        }
                        case 2: { // Read Discrete Inputs
                            ...
                            if (rc >= 0) {
                                int byte_count = (modbus_config.groups[i].reg_count + 7) / 8;
                                memcpy(modbus_data.discrete_inputs[i], bit_values, byte_count);
                            }
                            break;
                        }
                        case 3: { // Read Holding Registers
